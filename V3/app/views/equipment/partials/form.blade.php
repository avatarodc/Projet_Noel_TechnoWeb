            ...
            <form action="{{ $action }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" 
                           id="nom" 
                           name="nom" 
                           value="{{ isset($equipment) ? $equipment->getNom() : '' }}"
                ...
                <div class="mb-6">
                    ...
                    <select id="etat" 
                            ...
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm 
                                   focus:border-indigo-500 focus:ring-indigo-500"
                            required>
                        @foreach(['Neuf', 'Bon', 'Moyen', 'Usé'] as $option)
                            <option value="{{ $option }}"
                                {{ (isset($equipment) && $equipment->getEtat() == $option) ? 'selected' : '' }}>
                                {{ $option }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <div class="flex items-center">
                        <div class="flex items-center h-5">
                            <input type="checkbox" 
                                   id="disponibilite" 
                                   name="disponibilite"
                                   {{ (isset($equipment) && $equipment->getDisponibilite()) ? 'checked' : '' }}
